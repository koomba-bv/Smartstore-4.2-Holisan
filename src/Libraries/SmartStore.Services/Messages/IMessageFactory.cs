...
namespace SmartStore.Services.Messages
{
    ...
    public interface IMessageFactory
    {
        ...
        //object SendChangeAddressMessage(Customer customer, string email, string firstname, string sender, string phonenumber, LocalizedString subject, string body);
    }
    ...
    public static class IMessageFactoryExtensions
    {
        ...
        /// KB Change Address
        /// /// <summary>
        /// Sends the "ChangeAddress" message to the store owner
        /// </summary>
        public static CreateMessageResult SendChangeAddressMessage(
            this IMessageFactory factory, Customer customer, string senderEmail, string sender, string addresstype, string phonenumber, string subject, string message, int languageId = 0)
        {
            var model = new NamedModelPart("Message")
            {
                ["Subject"] = subject.NullEmpty(),
                ["Message"] = message.NullEmpty(),
                ["SenderEmail"] = senderEmail.NullEmpty(),
                ["SenderName"] = sender.HasValue() ? sender.NullEmpty() : senderEmail.NullEmpty(),
                ["SenderPhone"] = phonenumber.HasValue() ? phonenumber.NullEmpty() : senderEmail.NullEmpty(),
                ["AddressType"] = addresstype.HasValue()
            };

            var messageContext = MessageContext.Create(MessageTemplateNames.SystemChangeAddress, languageId, customer: customer);
