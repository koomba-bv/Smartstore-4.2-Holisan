...
using static SmartStore.Web.Models.ShoppingCart.ShoppingCartModel;
...
namespace SmartStore.Web.Controllers
{
    public partial class ShoppingCartController : PublicControllerBase
    {
            ...
            bool prepareAndDisplayOrderReviewData = false)
        {
            ...
            PrepareFreeProductsModel(model, cart);           
        ...
        // KOOMBA
        // Add free items for Ayuorder
        [NonAction]
        protected void PrepareFreeProductsModel(ShoppingCartModel model, IList<OrganizedShoppingCartItem> cart)
        {
            var discountFreeItems = _discountService.GetDiscountById(3);
            var freeProducts = discountFreeItems.AppliedToProducts.Where(x => !x.Deleted && x.Published).ToList();
            ...
            {
                decimal shoppingCartUnitPriceWithDiscountBase = _taxService.GetProductPrice(item.Item.Product, _priceCalculationService.GetUnitPrice(item, true), out taxRate);
                decimal shoppingCartUnitPriceWithDiscount = _currencyService.ConvertFromPrimaryStoreCurrency(shoppingCartUnitPriceWithDiscountBase, _workContext.WorkingCurrency);
                totalAmount += shoppingCartUnitPriceWithDiscount * item.Item.Quantity;
            }

            /// Add if freeproducts are shown based on the HasFreeProducts boolean
            /// if (cart.Any(c => c.Item.Product.HasFreeProducts && c.Item.Quantity >= c.Item.Product.MinProductAmount))
            
            // Now based upon the total amount in the cart
            if(totalAmount > 110)
            {     
                if (discountFreeItems?.AppliedToProducts == null || !discountFreeItems.AppliedToProducts.Any())
                    return;
            ...
            {
                var matchingFreeProduct = cart.FirstOrDefault(c => freeProducts.Any(fp => c.Item.ProductId == fp.Id));
                if (matchingFreeProduct != null)
                {
                    DeleteCartItem(matchingFreeProduct.Item.Id, false);
                }
                model.FreeProducts.FreeItems = null;
            }
        }

        // KOOMBA
        // Prepare FreeProductsPartialView
        [HttpGet]
        [Route("ShoppingCart/FreeProductsPartial")]
        public ActionResult FreeProductsPartial()
        {
            var currentStore = _storeContext.CurrentStore;
            var cart = _shoppingCartService.GetCartItems(_workContext.CurrentCustomer, ShoppingCartType.ShoppingCart, currentStore.Id);
            ...
            return PartialView("_FreeProducts", model.FreeProducts);
        }
        ...
        [HttpPost]
        [ValidateInput(false)]
        public ActionResult AddFreeProduct(int productId, int shoppingCartTypeId)
        {
            var product = _productService.GetProductById(productId);            
